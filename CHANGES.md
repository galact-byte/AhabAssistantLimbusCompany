...
## 2026-09-26 — 编队首屏选队与镜牢失败传播（未游戏实机验证）

### 背景与目标
- v1.5.11 在后台日常选队时，列表已归顶但强制下滚探针没有可见位移，04:04:26 中止；任务线程已退出而非死锁。上游 v1.6.0 同日能选队并进入战斗，但其旧拖动不能替代本地队伍身份核验。
- 顺序选队在已识别固定首项且目标可见时直接点击并核验顶部当前标题；滚到中途但首行几何像顶部时，必须看见固定首项才允许重置顺序。镜牢确认超时或恢复失败抛错给顶层，不误当成功继续。

### 影响与兼容性
- 修改 `tasks/team_list.py`、`tasks/teams/team_formation.py`、`tasks/mirror/mirror.py`，增补 `tests/test_team_list_parser.py`、`tests/test_team_visual_selection.py`；审计证据见 `.trellis/tasks/09-26-team-wheel-upstream-comparison/research/`。
- 保持按编号、Windows/模拟器现有输入、失败截图路径和用户配置不变；未触碰 G 盘安装版，不新增依赖。回滚本次代码及对应测试即可，无数据迁移。

### 验证与限制
- 原始失败 PNG 由项目同套 OCR 重放：固定首项成立、7 行、前两行中心 y=389.5/434.5。顶部无效滚轮、中途伪归顶与镜牢确认失败先 RED 后 GREEN；完整分支矩阵含日常、镜牢、按顺序、按编号、模拟器与输入失败路径。
- `.venv/Scripts/python.exe -m pytest -q`：322 passed；受影响代码及测试 Ruff 通过，镜牢文件 5 条 HEAD 已有 E712/F841 告警单列豁免，`git diff --check` 通过。
- 本轮没有向当前运行的游戏注入输入；后台非首屏滚轮在实机是否被消费、整夜挂机是否稳定尚未验证。非首屏滚轮停滞时仍有界失败，不恢复可能重排队伍的长按拖动；任务保留 active，不能把本地测试当作发行版已修好。
