...
- [ ] 主会话运行 `trellis-check` 完整核对与 `trellis-update-spec` 复盘，确认无需额外改动后提交本任务范围内的源码/测试/文档；新增用户授权后，推送 `main` 和 `v1.5.12` 标签，等待 `.github/workflows/release.yaml` 构建与 Release，回读正文和 `AALC_v1.5.12.7z` 资产。实机未验收时任务留在 active 而不归档。
