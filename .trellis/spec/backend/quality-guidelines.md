...
按顺序在首项名称明确为“剧情关卡”、首行确实位于标题下方顶部、列表逐行连续且目标可见时，无须额外下滚探针即可直接点击，但选后仍必须核对独立的顶部当前编队标题。目标不在当前页时先确认固定首项归顶；即使中途首行的几何位置像顶部，若没有固定项也不得按第N项选队。下滚/回顶探针只用于确实需要继续扫描的路径。禁止为了选第二项遍历全部40槽，40是编号上界而非实际槽数前提。只检查经过页面的歧义。
...
### 4. 错误矩阵
截图/识别不可用最多3次；单阶段滚动最多100次；点击核验最多3次。需要滚动却无法验证输入、缺行、重名、页间无重叠或标题不一致→False并保存可用失败帧；已知顶部首项且目标可见时无须发送滚轮。镜牢与日常不得继续确认/战斗。镜牢已经选中队伍但后续确认超时或恢复失败→抛 `cannotOperateGameError`，顶层返回 False。取消BaseException不能被吞掉。

### 5. 案例
按顺序第1/2项在900p首屏可直接核验；从中途卡住的列表必须拒绝伪归顶（无“剧情关卡”固定首项）。镜牢选队确认后未进入星光页或 `retry()` 失败应抛 `cannotOperateGameError`，由 `onetime_mir_process()` 返回 False，不应在 `Mirror.run()` 中空转。

### 6. 回归
`test_team_safe_input.py`禁止左键按住、验证wheel坐标/恢复/交互门；`test_team_list_parser.py`覆盖分区、缺行、标题，以及两张 900p 失败帧的首行坐标和标题重叠反例；`test_team_visual_selection.py` 覆盖本次失败帧的顶部1/2、首项不明、中途伪归顶、滚轮停滞、镜牢确认超时/恢复失败及成功路径；`test_daily_team_selection.py` 覆盖日常失败传播。模拟测试不代表1600×900实机滚轮生效。
